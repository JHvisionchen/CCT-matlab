function [rects, rect_features] = detect(im, weights, bias, ...
...
% 		end
   ...
   feat_rects = rects*scale_factor/cell_size;
    ...
    for cnt = 1:size(rects,1)
        tmp_rect = feat_rects(cnt,1:4);
        cx = (tmp_rect(1)+tmp_rect(3))/2;
        cy = (tmp_rect(2)+tmp_rect(4))/2;
        x_tl = ceil(cx - weight_sz(2)/2);
        y_tl = ceil(cy - weight_sz(1)/2);
        x_rb = x_tl + weight_sz(2);
        y_rb = y_tl + weight_sz(1);
        rect_features(:,:,:,cnt) = z(y_tl:y_rb-1, x_tl:x_rb-1,:);
%         if 1,
%             %weights visualization (positive on the left, negative on the right)
%             tmp_weights = rect_features(:,:,:,cnt);
%             w_norm = max(abs(tmp_weights(:)));
%             figure('Name', ['Weight range: ' num2str(w_norm) ', bias: ' num2str(bias)])
%             try  %#ok<ALIGN>
%                 imshow(vl_hog('render', 0.4 * single([max(0, tmp_weights/ w_norm), max(0, -tmp_weights / w_norm)])));
%             catch, end  %#ok<CTCH>
%         end
    end
    if 0
